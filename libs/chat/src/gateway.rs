...
use crate::messengers::telegram::{TelegramMessenger, TgCredentials, TgMirrors, TgSession};
...
impl MessengerGateway {
    ...
    pub async fn send(
        ...
        match platform {
            Platform::Telegram => {
                ...
                let session = TgSession {
                    cred,
                    mirrors: TgMirrors { urls: mirrors },
                };
        ...
        H: InboundHandler + Send + Sync + 'static,
    {
        ...
        tokio::spawn(async move {
            ...
            let _ = tg.ensure_polling_mode(&tg_session).await;
            let mut offset: i64 = 0;
            loop {
                match tg.fetch_messages(offset, tg_session.clone()).await {
